/**
 * WordPress Dependencie		console.log(
			'window._prc_nexus_pre_publish_check...',
			window._prc_nexus_pre_publish_check
		);/
...
// Hacky "global store" until we have time to make a proper @wordpress/data store in prc-nexus.
const setupStopPublishing = () => {
	if (typeof window._prc_nexus_pre_publish_check === 'undefined') {
		console.log('setting up window._prc_nexus_pre_publish_check...');
		window._prc_nexus_pre_publish_check = {
			blocks: [],
			stop: false,
		};
...
const stopPublishing = ({ edits, postId, postType }) => {
	...
	if (!window._prc_nexus_pre_publish_check) {
		return;
	}
	if (
		edits.status === 'publish' &&
		true === window._prc_nexus_pre_publish_check.stop
	) {
		window.alert(
			'✨ PRC Nexus assistance has been detected, so publishing is currently disabled. Your post will be reverted back to a draft. ✨'
		);
...
export const AInexusPrePublishCheck = () => {
	const blocks = useSelect((select) => {
		const { getBlocks } = select(blockEditorStore);
		const allBlocks = getBlocks();

		// Recursively find all blocks with _nexus metadata
		const findAIBlocks = (_blocks: any[]): AIBlock[] => {
			return _blocks.reduce((acc: AIBlock[], block) => {
				if (block.attributes?.metadata?._nexus) {
					acc.push({
	...
	// Hoist the blocks into _prc_nexus_pre_publish_check
	useEffect(() => {
		if (!blocks.length) {
			return;
		}
		setupStopPublishing();
		window._prc_nexus_pre_publish_check.blocks = blocks;
		window._prc_nexus_pre_publish_check.stop = true !== allBlocksReviewed;
	...
	return (
		<PluginPrePublishPanel
			title={__('PRC Nexus Editor Review', 'prc-block-library')}
			initialOpen={true}
			icon={false}
		>
			<div className="ai-prepublish-warning">
				<p>
					{__(
						'This post contains content generated by PRC Nexus. Please review the following blocks before publishing:',
						'prc-block-library'
					)}
				</p>
...
export default AInexusPrePublishCheck;
